...
export default class Game {
	...
	getRemainingTime() {
		const t = Math.ceil(
			(this.maxTime - (Date.now() - this.initialTime)) / 1000
		);
	...
	async vote(username, paragraph) {
		...
		if(pl.votedTo==""){
			...
			var value = 0;
			if (this.isPrivate) {
				value = POINTS_QK_PRIVATE;
			}
			else {
				value = POINTS_QK_PUBLIC;
			}
			receiver.stars = receiver.stars+value;
			receiver.mooncoins = receiver.mooncoins+value;
			rec.scored += value;
...
export async function checkEmpty(id) {
	...
	if (game != undefined) {
		...
		if (
			(elapsed > game.maxTime + 1000 * END_MARGIN &&
			...
			if (
				(game.paragraphs == 0 && game.state == state.INGAME) ||
