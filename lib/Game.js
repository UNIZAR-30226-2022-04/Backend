...
import { MARGIN } from "./GamesManager";
...
export default class Game {
	...
	getRemainingTime(){
		const t = (this.maxTime-(Date.now()-this.initialTime))/1000
		return ( t >= 0 ? t : 0);
	}
	...
	getStoryRanking(){
		var resul = [];
		this.players[this.voteTurn-1].paragraphs.forEach((p) =>{
			const aux = {
				body: p.body,
				username: p.creator
			}
	...
	async nextTurn() {
		...
		if (this.state == state.LOBBY) {
			...
			this.initialTime = this.initialTime-REVIEW_TIME
...
export function checkEmpty(id){
	...
	if (game != undefined){
		...
		if ((elapsed>(game.maxTime+1000*MARGIN) && game.state == state.INGAME)
