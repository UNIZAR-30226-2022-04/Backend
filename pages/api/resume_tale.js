	...
	if (user != undefined) {
		...
		if (user.password_hash == message.password) {
            ...
            if (tale != undefined) {
				...
				const canEdit = ((friendnames.indexOf(paragraphs[0].username) != -1) ||
                                (friendnames.indexOf(paragraphs[0].username) == message.username) ||
                                (tale.privacy == 0)) && (tale.max_turns > paragraphs.length) &&
                                (friendnames.indexOf(paragraphs[paragraphs.length-1].username) != message.username)

                if (canEdit){
                    res.status(200).json({ result: "success", title: tale.title, reason: "",
                                            paragraphs: paragraphs, maxCharacters: tale.max_paragraph_chars});
                } else {
                    res.status(200).json({ result: "error", reason: "cannot_access_tale" });
                }
            } else {
			    res.status(200).json({ result: "error", reason: "tale_doesnot_exist" });
            }
		} else {
